...
def _rope_config(config: LlamaConfig) -> tuple[float, dict | None]:
    rope_scaling = getattr(config, "rope_scaling", None) or getattr(config, "rope_parameters", None)
    rope_theta = getattr(config, "rope_theta", None) or 10000
    if isinstance(rope_scaling, dict):
        rope_theta = rope_scaling.get("rope_theta", rope_theta)
        # Strip rope_theta out of the scaling dict so the rope module sees only
        # scaling-relevant fields (matches transformers conventions).
        rope_scaling = {k: v for k, v in rope_scaling.items() if k != "rope_theta"}
        # Some configs use 'type' instead of 'rope_type'; normalize.
        if "rope_type" not in rope_scaling and "type" in rope_scaling:
            rope_scaling["rope_type"] = rope_scaling["type"]
        if not rope_scaling or rope_scaling.get("rope_type") in (None, "default"):
            rope_scaling = None
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # J1: streaming compact pack runs inline here, before flash_attn, so
        # the post-RoPE k/v are packed straight into compact arena and never
        # need a persistent dense scratch tensor. Only fires when DMS is
        # enabled and we have an active prefill batch.
        if (
            context.is_prefill
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        prof = get_profiler()
        with prof.cuda_phase("mlp_gate_up_total"):
            gate_up = self.gate_up_proj(x)
        with prof.cuda_phase("mlp_silu"):
            x = self.act_fn(gate_up)
        with prof.cuda_phase("mlp_down_total"):
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        prof = get_profiler()
        if residual is None:
            with prof.cuda_phase("rmsnorm_input"):
                hidden_states, residual = self.input_layernorm(hidden_states), hidden_states
        else:
            with prof.cuda_phase("rmsnorm_input"):
                hidden_states, residual = self.input_layernorm(hidden_states, residual)
        with prof.cuda_phase("self_attn_total"):
            hidden_states = self.self_attn(positions, hidden_states)
        with prof.cuda_phase("rmsnorm_post_attn"):
            hidden_states, residual = self.post_attention_layernorm(hidden_states, residual)
        with prof.cuda_phase("mlp_total"):
