...
def parse_env(items: list[str]) -> dict[str, str]:
    ...
    for item in items:
        ...
        key = key.strip()
        if not key:
            raise argparse.ArgumentTypeError(f"env override has empty key: {item!r}")
        parsed[key] = value
    return parsed
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-greedy-mismatches", type=int, default=8)
    parser.add_argument("--reference-env", action="append", default=[], metavar="KEY=VALUE")
    ...
    *,
    ...
    with temporary_env(env_updates, clear_nanovllm_env=args.clear_nanovllm_env):
        llm = LLM(
            ...
            max_num_batched_tokens=args.ctx_len * concurrency,
            ...
            dms_window_size=args.window_size,
        )
        ...
        try:
            while not llm.scheduler.is_finished():
                ...
                input_ids, positions = (
                    llm.model_runner.prepare_prefill(seqs)
                    if is_prefill
                    else llm.model_runner.prepare_decode(seqs)
                )
                try:
                ...
                for seq_idx, seq in enumerate(seqs):
                    step_logits = logits[seq_idx]
                    greedy = int(step_logits.argmax(dim=-1).item())
                    order_idx = seq_index[seq.seq_id]
                    step_i = decode_step_idx[seq.seq_id]
                    forced = forced_tokens[order_idx][step_i] if forced_tokens is not None else greedy
            ...
            metadata = {
                ...
                "env": env_updates,
                "window_size": (
                    llm.model_runner.dms_metadata.window_size
                    if llm.model_runner.dms_metadata is not None
                    else None
                ),
    ...
    return {
        ...
        "logits": torch.stack(logits_stacks, dim=0),
        **metadata,
    }
...
def decode_step_ppl(logits: torch.Tensor, target_token_ids: list[list[int]]) -> dict:
    ...
    return {
        ...
        "note": "decode-step PPL over reference greedy tokens, not teacher-forced WikiText next-token PPL",
    }
...
def main() -> None:
    ...
    reference = run_case(args, prompts, env_updates=reference_env, mode=args.reference_mode)
    candidate = run_case(
        args,
        prompts,
        env_updates=candidate_env,
